...
(() => {
  ...
  var _deWordify = function _deWordify2(e2) {
    var children = _toArray3(e2.childNodes), l = children.length, i, child;
    for (i = 0; i < l; i++) {
      child = children[i];
      if (child._isSplit) {
        _deWordify2(child);
  ...
  var _splitRawText = function _splitRawText2(element, vars, wordStart, charStart) {
    ...
    }, space = wordDelimiter !== " " ? "" : absolute ? "&#173; " : " ", wordEnd = "</" + tag + ">", wordIsOpen = 1, specialChars = vars.specialChars ? typeof vars.specialChars === "function" ? vars.specialChars : _findSpecialChars : null, text, splitText, i, j, l, character, hasTagStart, testResult, container = _doc6.createElement("div"), parent = element.parentNode;
  ...
  // src/fall_release.js
  gsapWithCSS.registerPlugin(ScrollTrigger2, SplitText, Flip);
  gsapWithCSS.config({
  ...
  function handleCode() {
    ...
    (() => {
      if (!document.querySelector(".section-fr-videos"))
        return;
    ...
    (() => {
      gsapWithCSS.from("[jump-start-block] [jump-start-block01]", {
        duration: 1,
        x: "2rem",
        scrollTrigger: "[jump-start-block]"
      });
      gsapWithCSS.from("[jump-start-block] [jump-start-block02]", {
        duration: 1,
        x: "-2rem",
        scrollTrigger: "[jump-start-block]"
      });
      gsapWithCSS.from("[supply-portal-block] [left-side-image]", {
        duration: 1,
        x: "100%",
        ease: "power1.out",
        scrollTrigger: "[supply-portal-block]"
      });
      gsapWithCSS.from("[supply-portal-block] [right-side-image]", {
        duration: 1,
        x: "-100%",
        ease: "power1.out",
        scrollTrigger: "[supply-portal-block]"
    ...
    (() => {
      if (!document.querySelector("[show-letter-modal]"))
        return;
    ...
    (() => {
      ...
      if (checkCookie("loading-animation")) {
        ...
        tl.to(loadingLogo, { scale: 2 }).call(() => {
        ...
        }).to(loadingLogo, { duration: 1, color: "#f53" }, "<").to(
          ".loading-screen",
          {
            duration: 1,
          ...
          document.querySelector("[logo-flip-item-wrapper]").appendChild(document.querySelector("[loading-screen-logo-wrapper]"));
          Flip.from(state, {
            duration: 0.65,
            ease: "power1.inOut",
            onComplete: () => {
    ...
    setTimeout(() => {
      if (!document.querySelector(".section-fr-unlock"))
        return;
      let mySplitText = new SplitText("[split-text]", { type: "words,chars" });
      gsapWithCSS.to(mySplitText.words, {
        color: "#1a2b49",
        stagger: 0.05,
        scrollTrigger: {
      ...
      gsapWithCSS.from(".unlock-huge-header", {
        scale: 2,
        scrollTrigger: {
          trigger: ".unlock-heading-track",
          start: "top 80%",
    ...
    (() => {
      if (!document.querySelector("[three-imgs-wrapper]"))
        return;
      let tl = gsapWithCSS.timeline();
  ...
  function handlePopups() {
    ...
    popupSections.forEach((popupSection) => {
      ...
      let openVideoPopup = (popup2) => {
        return gsapWithCSS.to(popup2, { duration: 0.5, ease: "ease", opacity: 1, pointerEvents: "auto" });
      };
      let closeVideoPopup = (popup2) => {
        return gsapWithCSS.to(popup2, { duration: 0.5, ease: "ease", opacity: 0, pointerEvents: "none" });
      };
  ...
  function handleSlider() {
    if (document.querySelector("[tab-video-slider]")) {
      let activateButtons2 = function(slideIndex) {
      ...
      var activateButtons = activateButtons2;
      var mySwiper = new Swiper("[tab-video-slider]", {
        ...
        on: {
          init: function() {
            ...
            activateButtons2(activeSlideIndex);
          },
      ...
      document.querySelectorAll(".tab-btn").forEach(function(button) {
        button.addEventListener("click", function() {
          ...
          activateButtons2(slideIndex);
        });
      });
    }
    if (document.querySelector("[stories-slider]")) {
      let updateSlideInfo2 = function(swiper2) {
        const currentSlide = swiper2.realIndex + 1;
        document.querySelector("[stories-slider] [current-slide]").textContent = currentSlide < 10 ? `0${currentSlide}` : currentSlide;
        const totalSlides = swiper2.slides.length;
        document.querySelector("[stories-slider] [total-slides]").textContent = totalSlides < 10 ? `0${totalSlides}` : totalSlides;
      }, updateNextSlideHeading2 = function(swiper2) {
        let nextSlideIndex = swiper2.realIndex + 1;
        if (nextSlideIndex >= swiper2.slides.length) {
          nextSlideIndex = 0;
        }
        const nextSlide = swiper2.slides[nextSlideIndex];
        const nextSlideHeading = nextSlide.querySelector("[slide-heading]");
        if (nextSlideHeading) {
          document.querySelector("[stories-slider] [next-slide-heading]").textContent = nextSlideHeading.textContent;
        }
      };
      var updateSlideInfo = updateSlideInfo2, updateNextSlideHeading = updateNextSlideHeading2;
      var swiper = new Swiper("[stories-slider]", {
        ...
        on: {
          init: function() {
            updateSlideInfo2(this);
            updateNextSlideHeading2(this);
          },
          slideChange: function() {
            updateSlideInfo2(this);
            updateNextSlideHeading2(this);
...
gsap/SplitText.js:
  ...
  *)
*/
//# sourceMappingURL=fall_release.js.map
